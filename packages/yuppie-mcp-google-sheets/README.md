...
本包是 MCP 壳：核心 Google Sheets 客户端代码在库包 `yuppie-google-sheets`（PyPI 分发，`pip install yuppie-google-sheets`），本包依赖它。

如只需在 Python 项目中操作 Google Sheets（不要 MCP），直接装库包：

```bash
uv pip install yuppie-google-sheets
```
